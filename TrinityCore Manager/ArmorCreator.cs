...
{
    public partial class ArmorCreator : DevComponents.DotNetBar.Office2007Form
    {
        public ArmorCreator()
        {
            InitializeComponent();
        ...
        private void WeaponCreator_Load(object sender, EventArgs e)
        {
            ...
            materialComboBoxEx.SelectedIndex = 0;
            ...
            else
            {
                ...
                MySQLConnLabelX.Visible = true;
                MySQLConnLabelX.Text = "Getting Armor Information...";

                armorInfoBackgroundWorker.RunWorkerAsync();
            }
        }

        private void wizard_WizardPageChanging(object sender, WizardCancelPageChangeEventArgs e)
        {
            if (e.OldPage == armorDetailsWizardPage && e.PageChangeSource == eWizardPageChangeSource.NextButton)
            {
                if (nameTextBoxX.Text == String.Empty || quoteTextBoxX.Text == String.Empty || entryIDIntegerInput.Value == 0 || displayIDIntegerInput.Value == 0)
                    e.Cancel = true;
            }
        }
        ...
        Dictionary<string, List<string>> armorInfo = null;

        private void armorInfoBackgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                if (mysql != null)
                    armorInfo = mysql.ReadAll(Settings.Default.WorldDB, "item_template", new string[] { "entry" }, new string[] { editItemID.ToString() });
            }
            catch
            {
            }
        ...
        private void armorInfoBackgroundWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            MySQLConnProgressBarX.Enabled = false;
            MySQLConnProgressBarX.Visible = false;
            ...
            if (armorInfo != null && armorInfo.ContainsKey("entry") && armorInfo["entry"].Count == 1)
            {
                if (int.Parse(armorInfo["class"][0]) == 4)
                {

                    wizardPage1.NextButtonEnabled = eWizardButtonState.True;

                    entryIDIntegerInput.Value = int.Parse(armorInfo["entry"][0]);
                    nameTextBoxX.Text = armorInfo["name"][0];
                    quoteTextBoxX.Text = armorInfo["description"][0];
                    qualityComboBoxEx.SelectedIndex = int.Parse(armorInfo["Quality"][0]);
                    displayIDIntegerInput.Value = int.Parse(armorInfo["displayid"][0]);
                    
                    switch (int.Parse(armorInfo["subclass"][0]))
                    {
                        case 10:
                            materialComboBoxEx.SelectedIndex = 9;
                            break;
                        case 13:
                            materialComboBoxEx.SelectedIndex = 10;
                            break;
                        case 15:
                            materialComboBoxEx.SelectedIndex = 11;
                            break;
                        case 16:
                            materialComboBoxEx.SelectedIndex = 12;
                            break;
                        case 17:
                            materialComboBoxEx.SelectedIndex = 13;
                            break;
                        case 18:
                            materialComboBoxEx.SelectedIndex = 14;
                            break;
                        case 19:
                            materialComboBoxEx.SelectedIndex = 15;
                            break;
                        default:
                            materialComboBoxEx.SelectedIndex = int.Parse(armorInfo["subclass"][0]);
                            break;
                    }
                    
                    switch (int.Parse(armorInfo["InventoryType"][0]))
                    {
                        case 13:
                            equipComboBoxEx.SelectedIndex = 0;
                            break;
                    ...
                    durabilityIntegerInput.Value = int.Parse(armorInfo["MaxDurability"][0]);
                    bindsComboBoxEx.SelectedIndex = int.Parse(armorInfo["bonding"][0]);


                    blockIntegerInput.Value = int.Parse(armorInfo["block"][0]);
                    armorIntegerInput.Value = int.Parse(armorInfo["armor"][0]);

                    int statValueCtr = 0;

                    if (int.Parse(armorInfo["stat_type1"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type2"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type3"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type4"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type5"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type6"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type7"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type8"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type9"][0]) != 0)
                        statValueCtr += 1;
                    if (int.Parse(armorInfo["stat_type10"][0]) != 0)
                        statValueCtr += 1;
                            ...
                            if (c.Name.Contains("statTypeComboBoxEx"))
                            {
                                for (int x = 0; x < 9; x++)
                                {
                                    if (c.Name == String.Format("statTypeComboBoxEx{0}", x + 1))
                                    {
                                        ...
                                        switch (int.Parse(armorInfo[String.Format("stat_type{0}", z)][0]))
                                        {
                                            case 1:
                                                statType = 0;
                                                break;
                        ...
                        else if (c is DevComponents.Editors.IntegerInput)
                        {
                            for (int x = 0; x < 9; x++)
                            {
                                if (c.Name == String.Format("statTypeValueIntegerInput{0}", x + 1))
                                {
                                    ...
                                    integerInput.Value = int.Parse(armorInfo[String.Format("stat_value{0}", w)][0]);
                                }
                            }
                    ...
                    requiredLevelIntegerInput.Value = int.Parse(armorInfo["RequiredLevel"][0]);
                    itemLevelIntegerInput.Value = int.Parse(armorInfo["ItemLevel"][0]);
                    maxAllowedIntegerInput.Value = int.Parse(armorInfo["maxcount"][0]);
                    stackableIntegerInput.Value = int.Parse(armorInfo["stackable"][0]);


                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 1) != 0)
                        warriorCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 2) != 0)
                        paladinCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 4) != 0)
                        hunterCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 8) != 0)
                        rogueCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 16) != 0)
                        priestCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 32) != 0)
                        deathKnightCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 64) != 0)
                        shamanCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 128) != 0)
                        mageCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 256) != 0)
                        warlockCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableClass"][0]) & 1024) != 0)
                        druidCheckBoxX.Checked = true;

                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 1) != 0)
                        humanCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 2) != 0)
                        orcCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 4) != 0)
                        dwarfCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 8) != 0)
                        nightElfCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 16) != 0)
                        undeadCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 32) != 0)
                        taurenCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 64) != 0)
                        gnomeCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 128) != 0)
                        trollCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 512) != 0)
                        bloodElfCheckBoxX.Checked = true;
                    if ((int.Parse(armorInfo["AllowableRace"][0]) & 1024) != 0)
                        draeneiCheckBoxX.Checked = true;


                    holyIntegerInput.Value = int.Parse(armorInfo["holy_res"][0]);
                    natureIntegerInput.Value = int.Parse(armorInfo["nature_res"][0]);
                    shadowIntegerInput.Value = int.Parse(armorInfo["shadow_res"][0]);
                    fireIntegerInput.Value = int.Parse(armorInfo["fire_res"][0]);
                    frostIntegerInput.Value = int.Parse(armorInfo["frost_res"][0]);
                    arcaneIntegerInput.Value = int.Parse(armorInfo["arcane_res"][0]);

                    switch (int.Parse(armorInfo["socketColor_1"][0]))
                    {
                        case 1:
                    ...
                    switch (int.Parse(armorInfo["socketColor_2"][0]))
                    {
                        case 1:
                    ...
                    switch (int.Parse(armorInfo["socketColor_3"][0]))
                    {
                        case 1:
                    ...
                    switch (int.Parse(armorInfo["socketBonus"][0]))
                    {
                        case 3312:
                    ...
                    int buyTotal = int.Parse(armorInfo["BuyPrice"][0]);
                    ...
                    buyTotal = int.Parse(armorInfo["SellPrice"][0]);
                ...
                {
                    TaskDialog.Show(new TaskDialogInfo("Error", eTaskDialogIcon.Stop, "Error!", "This entry id does not belong to armor!", eTaskDialogButton.Ok));
            ...
            {
                TaskDialog.Show(new TaskDialogInfo("Error", eTaskDialogIcon.Stop, "Error!", "Armor not found!", eTaskDialogButton.Ok));
        ...
        {
            SearchItemID searchArmorID = new SearchItemID();
            searchArmorID.FormClosed += new FormClosedEventHandler(searchArmorID_FormClosed);

            searchArmorID.searchType = SearchItemID.SearchType.Armor;

            searchArmorID.ShowDialog();
        }

        private void searchArmorID_FormClosed(object sender, FormClosedEventArgs e)
        {
            SearchItemID searchArmorID = (SearchItemID)sender;

            displayIDIntegerInput.Value = searchArmorID.GetDisplayID();
        }
        ...
        private void wizard_FinishButtonClick(object sender, CancelEventArgs e)
        {
            ...
            try
            {
                int subclass = materialComboBoxEx.SelectedIndex + 1;
                int inventorytype = 0;
                int socket_bonus = 0;
                ...
                string armorName = nameTextBoxX.Text.Replace("'", "\\'");
                string armorQuote = quoteTextBoxX.Text.Replace("'", "\\'");

                string[] columns = new string[] { "entry", "class", "subclass", "name", "displayid", "Quality", "BuyPrice", "SellPrice", "InventoryType", "AllowableClass", "AllowableRace", "ItemLevel", "RequiredLevel", "maxcount", "stackable", "StatsCount", "stat_type1", "stat_value1", "stat_type2", "stat_value2", "stat_type3", "stat_value3", "stat_type4", "stat_value4", "stat_type5", "stat_value5", "stat_type6", "stat_value6", "stat_type7", "stat_value7", "stat_type8", "stat_value8", "stat_type9", "stat_value9", "stat_type10", "stat_value10", "armor", "holy_res", "fire_res", "nature_res", "frost_res", "shadow_res", "arcane_res", "bonding", "description", "Material", "block", "MaxDurability", "socketColor_1", "socketColor_2", "socketColor_3", "socketBonus" };
                string[] values = new string[] { entryIDIntegerInput.Value.ToString(), "4", subclass.ToString(), armorName, displayIDIntegerInput.Value.ToString(), qualityComboBoxEx.SelectedIndex.ToString(), buymoney.ToString(), sellmoney.ToString(), inventorytype.ToString(), allowableclass.ToString(), allowablerace.ToString(), itemLevelIntegerInput.Value.ToString(), requiredLevelIntegerInput.Value.ToString(), maxAllowedIntegerInput.Value.ToString(), stackableIntegerInput.Value.ToString(), statsAmount.ToString(), statType1.ToString(), statValue1.ToString(), statType2.ToString(), statValue2.ToString(), statType3.ToString(), statValue3.ToString(), statType4.ToString(), statValue4.ToString(), statType5.ToString(), statValue5.ToString(), statType6.ToString(), statValue6.ToString(), statType7.ToString(), statValue7.ToString(), statType8.ToString(), statValue8.ToString(), statType9.ToString(), statValue9.ToString(), statType10.ToString(), statValue10.ToString(), armorIntegerInput.Value.ToString(), holyIntegerInput.Value.ToString(), fireIntegerInput.Value.ToString(), natureIntegerInput.Value.ToString(), frostIntegerInput.Value.ToString(), shadowIntegerInput.Value.ToString(), arcaneIntegerInput.Value.ToString(), bindsComboBoxEx.SelectedIndex.ToString(), armorQuote, materialComboBoxEx.SelectedIndex.ToString(), blockIntegerInput.Value.ToString(), durabilityIntegerInput.Value.ToString(), socket_1.ToString(), socket_2.ToString(), socket_3.ToString(), socket_bonus.ToString() };
                ...
                if (saveToDBRadioButton.Checked)
                {
                    ...
                    TaskDialog.Show(new TaskDialogInfo("Done", eTaskDialogIcon.Information2, "Armor imported into database", String.Empty, eTaskDialogButton.Ok));
